...
class NaverDiscussionCrawler(NaverFinanceCrawler):
    ...
    def parse_page(self, code, page_idx):
        ...
        for info_soup in info_soup_list:
            ...
            strong_strong = int(strong_list[0].text)
            strong_weak = int(strong_list[1].text)

            info_dict = {"ts":int(ts), "view_count":int(view_count), "strong":int(strong_strong), "weak":int(strong_weak)}
...
class MarketBuyerInfoCrawler(NaverFinanceCrawler):
    ...
    def parse_page(self, page_idx):
        ...
        for info_soup in info_soup_list:
            ...
            date_slash_string = "20" + date_dot_string.replace(".","/")

            ant_net_amount = int(info_array[1].text.replace(",",""))
            foreigner_net_amount = int(info_array[2].text.replace(",",""))
            institue_net_amount = int(info_array[3].text.replace(",",""))            
            finance_net_amount = int(info_array[4].text.replace(",",""))
            ...
            info_dict_list.append({
                ...
                "institue_net_amount":institue_net_amount,
                "finance_net_amount" : finance_net_amount
            })
...
if __name__ == "__main__":
    ...
    res = ndc.get_result_data('041830', 1, 5)
    print(res)
    """

    mbc = MarketBuyerInfoCrawler()
    res = mbc.get_result_data(2, 10)
    save_result_data(res, 'result.csv')
